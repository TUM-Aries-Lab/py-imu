...
def gyr_bias_cal():
    ...
    for ii in range(0, 3):
        axs[0].plot(data[:, ii], label=f"${gyr_labels[ii]}$, Uncalibrated")
        axs[1].plot(
            data[:, ii] - gyr_offsets[ii],
            label=f"${gyr_labels[ii]}$, Calibrated",
        )
    axs[0].legend(fontsize=14)
    axs[1].legend(fontsize=14)
    axs[0].set_ylabel(r"$w_{x,y,z}$ [$^{\circ}/s$]", fontsize=18)
    axs[1].set_ylabel(r"$w_{x,y,z}$ [$^{\circ}/s$]", fontsize=18)
    axs[1].set_xlabel("Sample", fontsize=18)
    axs[0].set_ylim([-2, 2])
    axs[1].set_ylim([-2, 2])
    axs[0].set_title("Gyroscope Calibration Offset Correction", fontsize=22)
    fig.savefig(
        "gyro_calibration_output.png", dpi=300, bbox_inches="tight", facecolor="#FCFCFC"
    )
    fig.show()
...
def gyr_drift(offset):
    ...
    #
    # print out results
    print(f"Integration of x in {integ_x[-1]:2.2f}m")
    print(f"Integration of y in {integ_y[-1]:2.2f}m")
    print(f"Integration of z in {integ_z[-1]:2.2f}m")
    ...
    axs[0].set_ylabel(
        r"Angular Velocity, $\omega_{}$ [$^\circ/s$]".format(
            gyro_labels[rot_axis].split("_")[1]
        ),
        fontsize=16,
    )
    axs[1].set_ylabel(
        r"Rotation, $\theta_{}$ [$^\circ$]".format(gyro_labels[rot_axis].split("_")[1]),
        fontsize=16,
    )
    axs[1].set_xlabel("Time [s]", fontsize=16)
    axs[0].set_title(r"Gyroscope Integration over 180$^\circ$ Rotation", fontsize=18)
    fig.savefig(
        "gyroscope_integration_180deg_rot.png",
        dpi=300,
        bbox_inches="tight",
        facecolor="#FFFFFF",
    )
